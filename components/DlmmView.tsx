...
function PrettyTooltip({ active, label, payload }: MyTooltipProps) {
  ...
  const lbl = typeof label === 'number' ? label.toFixed(4) : String(label ?? '');
    ...
    <section className="w-full space-y-6">
        ...
        <div className="card glass p-5">
          ...
          {connected ? (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs muted">— {shortPk}</span>
            </div>
          ) : (
            <ConnectWallet />
          )}
      ...
      <div className="card glass p-6">
        ...
        <p className="text-xs muted mt-2">
          Currently using a stub for bins. If the upstream DLMM endpoint can&apos;t be reached or the pool is invalid,
          demo bins are shown as a fallback.
        </p>
      </div>
